...
# 🗑 前に入れたファイルを消す手順（ブルービーン）。オートコール投入が、投入の前に必ず使う
#    （同じ業務・同じシート名の前のファイルを探して、確認なしで消す）。
#    ⚠️ 一度は別のボタン（過去リスト削除）に分けたが、時間指定の自動実行で最後まで通せないので、投入の流れに戻した。
...
def bb_delete_block(supabase, row, steps, key: str):
    ...
    if any(str(s.get("操作", "")) == BB_DELETE_OP for s in steps):
        st.caption("✅ 投入の前に、同じ業務・同じシート名で前に入れたファイルを探して消します。")
        return
    st.warning("⚠️ ロボットに**前のファイルを消す手順**がありません。オートコール投入は、"
               "この手順が無いと動きません（前のファイルが残ったまま入れると処理失敗になるため）。")
