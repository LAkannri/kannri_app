...
if not (_has_secret("SUPABASE_URL") and _has_secret("SUPABASE_KEY")):
    ...
    """)
    st.page_link("pages/99_⚙️_その他設定.py", label="⚙️ 設定の手順を見る（カンナの部屋へ）", use_container_width=True)
    st.stop()
...
_ACTION_VERB = {
    ...
    "印のある行を繰り返す": "が入っている行だけを、上から順に処理します（「ここまで繰り返す」までを繰り返します）",
    "repeat_rows": "が入っている行だけを、上から順に処理します（「ここまで繰り返す」までを繰り返します）",
    "ここまで繰り返す": "：ここまでを1行ぶんとして繰り返します",
    "repeat_end": "：ここまでを1行ぶんとして繰り返します",
...
# ==========================================
    ...
    if target_url:
        ...
        # 📌 ⚠️ 貼り付けた直後にボタンを押すと、**押した操作が捨てられる**ことがある。
            ...
            edited_df = st.data_editor(df, num_rows="dynamic", use_container_width=True, key=f"editor_{project_id}",
                                       ...
                                       column_config={
                                           ...
                                           "対象": st.column_config.TextColumn(
                                               "対象（画面の欄）",
                                               help="画面のどこを操作するか。文字で書きます。"
                                                    "ファイル名が毎回変わるサイトでは「最新のファイル」と書くと、"
