...
@st.cache_data(ttl=120, show_spinner=False)
def _read_sheet_rows(_gc, sheet_id: str, tab: str, limit: int = 300):
    """1シートを読んで (見出し, 行) で返す。少しの間おぼえておく。"""
    ws = _gc.open_by_key(str(sheet_id).strip()).worksheet(str(tab).strip())
    vals = ws.get_all_values()
    if not vals:
        return [], []
    hdr = [str(h).strip() for h in vals[0]]
    body = [(r + [""] * len(hdr))[:len(hdr)] for r in vals[1:]
            if any(str(c).strip() for c in r)]
    return hdr, body[:limit]
...
def _save_pick_order(gc, url, df_all, sid, picked):
    """実行画面で選んだ順を、設定シートの「順番」として保存する。
    ⚠️ 動かすのは、その貼り付け先スプシの行だけ（他のスプシの並びは触らない）。
    ⚠️ 選ばなかったキャリアは後ろに回す（外した日があっても設定は消えない）。
    ⚠️ 番号は**全行に1から振り直す**こと。空のままの行は「いちばん後ろ」扱いになるので、
       一部だけ番号を持つと、並びが直感と食い違う。"""
    rows = [r.to_dict() for _, r in df_all.iterrows()]
    slots = [i for i, r in enumerate(rows)
             if str(r.get("貼り付け先スプシID", "")).strip() == str(sid).strip()]
    names = [str(rows[i].get("キャリア名", "")) for i in slots]
    by_name = {n: rows[i] for i, n in zip(slots, names)}
    order = ([n for n in picked if n in by_name]
             + [n for n in names if n not in picked])
    for slot, name in zip(slots, order):
        rows[slot] = by_name[name]
...
# ==========================================
# 画面の切り替え（ふだんは「実行」だけ。設定は奥にしまう）
# ==========================================
# 📌 設定はいちど決めたら触らないので、ふだんの画面から見えないようにする。
#    毎日使うのは「実行」と「確認」だけ。
st.session_state.setdefault("pg_view", "main")
_h1, _h2 = st.columns([5, 1])
with _h2:
    if st.session_state.pg_view == "main":
        if st.button("⚙️ 設定", use_container_width=True, key="pg_to_settings",
                     help="キャリアの登録・取り込み方の設定はこちら"):
            st.session_state.pg_view = "settings"
            st.rerun()
    else:
        if st.button("⬅ 実行の画面へ", use_container_width=True, type="primary",
                     key="pg_to_main"):
            st.session_state.pg_view = "main"
            st.rerun()
            ...
            if df is not None:
                ...
                if not _editing:
                    ...
                    if not _carriers:
                        ...
                        # 🔢 何番目に実行されるかは、**同じ貼り付け先スプシの中での順**で数える
                        #    （実行画面はスプシごとに分かれているため。通し番号で出すと食い違う）。
                        _seq, _cnt = {}, {}
                        for _c in _carriers:
                            _b = str(df[df["キャリア名"] == _c].iloc[0]
                                     .get("貼り付け先スプシID", "")).strip()
                            _cnt[_b] = _cnt.get(_b, 0) + 1
                            _seq[_c] = _cnt[_b]
                        _cols = st.columns(3)
                        for _i, _cname in enumerate(_carriers):
                            ...
                            with _cols[_i % 3]:
                                with st.container(border=True):
                                    ...
                                    st.caption(f"🔢 実行は {_seq[_cname]} 番目"
                                               "（順番は実行画面で変えます）")
...
if st.session_state.pg_view == "main":
        ...
        if not (gc and cfg.get("settings_url")):
            ...
            for _sid, _members in _groups.items():
                ...
                with st.container(border=True):
                    ...
                    with _pc1:
                        # ⭐ ここは「どれを実行するか」と「どの順で実行するか」の両方を決める欄。
                        #    ⚠️ 「まとめて反映してから、それを集めた1枚を投入する」という前後関係が
                        #       あるので（ニチガスの反映 → ニチガスのデータローダー）、
                        #       **選んだ順がそのまま実行の順**になる。
                        _picked = st.multiselect("どのキャリアを実行する？（選んだ順に実行します）",
                                                 _all_names, default=_all_names, key=f"pick_{_sid}",
                                                 help="上から選んだ順に、取り込み → 貼り付け → "
                                                      "Salesforceへの投入まで行います")
                    with _pc2:
                        st.markdown("<div style='height:1.8rem'></div>", unsafe_allow_html=True)
                        if st.button("すべて選択", key=f"pickall_{_sid}", use_container_width=True):
                            st.session_state[f"pickset_{_sid}"] = _all_names
                            st.rerun()
                        if st.button("ぜんぶ外す", key=f"pickclr_{_sid}", use_container_width=True,
                                     help="順番を組み直すときに押します"):
                            st.session_state[f"pickset_{_sid}"] = []
                            st.rerun()
                    # ⚠️ 並べ替えは「いったん外して、やりたい順に選び直す」で行う。
                    #    ふつうは全部のままでよく、順番を変えるのはまれなので、
                    #    そのためだけの部品を増やさない。
                    if _picked:
                        st.caption("▶ 実行の順番：" +
                                   " → ".join(f"{_n + 1}. {_c}" for _n, _c in enumerate(_picked)))
                    # ⭐ この順番は、押したときだけ保存する（次に開いたときも同じ並びで出る）。
                    #    毎回勝手に保存すると、「1社だけやり直す」で外した並びが残ってしまう。
                    # ⚠️ 保存できるのは**全部を選び直したとき**だけ。
                    #    一部だけ選んだ状態で保存できると、「今日1社だけやり直す」つもりの
                    #    選択が、明日からの並びとして残ってしまう（外したキャリアが後ろへ飛ぶ）。
                    if _picked and sorted(_picked) == sorted(_all_names):
                        if list(_picked) != _all_names and st.button(
                                "💾 この順番で保存する", key=f"picksave_{_sid}",
                                use_container_width=True,
                                help="次に開いたときも、この並びで出ます"):
                            try:
                                _save_pick_order(gc, cfg["settings_url"], _rows_all,
                                                 _sid, list(_picked))
                        ...
                        st.caption("💡 順番を保存するには、**ぜんぶ外す → やりたい順に選び直す**で"
                                   "全キャリアを並べてください（いまは一部だけ選ばれています）。"
                                   "このまま実行する分には、上の順番で動きます。")
                    # ⭐ 実行も投入も、**選んだ順**で行う
                    _by_name = {str(m["キャリア名"]): m for m in _members}
                    _members = [_by_name[_n] for _n in _picked if _n in _by_name]
                    if not _members:
    ...
    # 確認用シートを、まとめて見る
    # ==========================================
    # 📌 これまでは1キャリアずつ選んで開くしかなく、
    #    「どこに何件出ているか」を知るのに何度も操作が要った。
    #    まず**件数を一覧**にして、中身が出ているものだけ開けるようにする。
    with st.container(border=True):
        theme.section_title("🔎", "確認用シートをまとめて見る")
        st.caption("各キャリアの**確認用シート**に何件出ているかを、いっぺんに出します。"
                   "0件なら見る必要はありません。中身があるものだけ開いて確かめてください。")
        if not (gc and cfg.get("settings_url")):
            st.info("先に「⚙️ 設定」で、設定スプレッドシートとキャリアを登録してください。")
        else:
            try:
                _crows = _read_config_rows(gc, cfg["settings_url"])
            except Exception as _e:
                _crows = pd.DataFrame(columns=CONFIG_HEADERS)
                st.error(f"設定を読めませんでした: {_e}")
            _clive = _crows[_crows["有効"].astype(str).str.upper() != "FALSE"] \
                if len(_crows) else _crows
            _targets = [r.to_dict() for _, r in _clive.iterrows()
                        if str(r.get("確認用シート名", "")).strip()
                        and str(r.get("貼り付け先スプシID", "")).strip()]
            if not _targets:
                st.info("確認用シートが設定されているキャリアがありません"
                        "（⚙️ 設定 → キャリアの設定 で「確認用シート名」を入れてください）。")
            else:
                _b1, _b2 = st.columns([1, 3])
                with _b1:
                    if st.button("🔄 ぜんぶ読み込む", type="primary",
                                 use_container_width=True, key="chk_all"):
                        _read_sheet_rows.clear()
                        st.session_state["chk_loaded"] = True
                        st.rerun()
                with _b2:
                    st.caption(f"対象：{len(_targets)}キャリア"
                               "／読み込むと少し時間がかかります（1キャリアずつ読むため）")
                if st.session_state.get("chk_loaded"):
                    # 貼り付け先のスプシ（＝LL／N など）ごとにまとめる
                    _by_book = {}
                    for _t in _targets:
                        _by_book.setdefault(str(_t["貼り付け先スプシID"]).strip(), []).append(_t)
                    for _sid, _mem in _by_book.items():
                        try:
                            _bt = gc.open_by_key(_sid).title
                        except Exception:
                            _bt = _sid[:12] + "…"
                        st.markdown(f"**📗 {_bt}**")
                        _sum, _hits = [], []
                        for _t in _mem:
                            _name = str(_t["キャリア名"])
                            _tab = str(_t["確認用シート名"]).strip()
                            try:
                                _hdr, _body = _read_sheet_rows(gc, _sid, _tab)
                                _sum.append({"キャリア": _name, "確認用シート": _tab,
                                             "件数": len(_body),
                                             "状態": ("✅ なし" if not _body else "🛠 要確認")})
                                if _body:
                                    _hits.append((_name, _tab, _hdr, _body))
                            except Exception as _e:
                                _sum.append({"キャリア": _name, "確認用シート": _tab,
                                             "件数": -1,
                                             "状態": f"❌ 読めません（{str(_e)[:40]}）"})
                        st.dataframe(pd.DataFrame(_sum), use_container_width=True,
                                     hide_index=True)
                        if not _hits:
                            st.success("✅ このスプレッドシートは、確認するものがありません。")
                        for _name, _tab, _hdr, _body in _hits:
                            with st.expander(f"🛠 {_name}／{_tab}（{len(_body)}件）",
                                             expanded=len(_hits) == 1):
                                st.caption("⚠️ 実データのため個人情報が含まれます")
                                st.dataframe(
                                    pd.DataFrame(_body,
                                                 columns=[h or f"列{i + 1}"
                                                          for i, h in enumerate(_hdr)]),
                                    use_container_width=True, hide_index=True)
                                _row = _clive[_clive["キャリア名"] == _name]
                                if len(_row):
                                    st.markdown(
                                        f"[📝 このシートを開いて直す]"
                                        f"(https://docs.google.com/spreadsheets/d/{_sid}/edit)")
                else:
                    st.caption("「🔄 ぜんぶ読み込む」を押すと、ここに一覧が出ます。")
