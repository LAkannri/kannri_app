...
- ⏳ **`終わるまで待つ` ステップ**（`robot.py` の `wait_done`）：更新はレポートによって
  何分もかかる。待たずに次のシートへ移ると更新を打ち切ってしまうので、必ず入れる。
  `対象`＝処理中だけ出る文字（消えるまで待つ／15秒出なければ済んだとみなす）、
  `値`＝最大何秒（既定300）。`対象` が空ならその秒数ただ待つ。
  時間切れは**失敗として止める**（取りこぼしたまま進めない）。登録画面が未設定を警告する。
